...
mkdir -p "$WRAPPER/sequence_10/unscaled" "$WRAPPER/sequence_10/scaled"
python scripts/evaluate_deepdct_vo.py "${COMMON[@]}" \
  --translation-scale-factor 1.0 \
  --output-dir "$WRAPPER/sequence_10/unscaled" | tee "$WRAPPER/logs/sequence_10_unscaled.log"
python scripts/evaluate_deepdct_vo.py "${COMMON[@]}" \
  --translation-scale-factor 1.007 \
  --output-dir "$WRAPPER/sequence_10/scaled" | tee "$WRAPPER/logs/sequence_10_scaled.log"
